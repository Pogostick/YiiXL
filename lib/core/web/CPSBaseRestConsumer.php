 ...
 * onRequestComplete
 *
 * Each are called respectively and pass the handler a CXLHelperBase
 */
class CXLHelperBase
{
	//********************************************************************************
	...
	public function init()
	{
		...
		//	Attach our events...
		$this->attachEventHandler( 'onBeforeRestCall', array( $this, 'beforeRestCall' ) );
		$this->attachEventHandler( 'onAfterRestCall', array( $this, 'afterRestCall' ) );
	...
	protected function makeRequest( $subType = '/', $requestData = null, $requestMethod = 'GET' )
	{
		...
		//	Handle events...
		$_event = new CXLHelperBase
		$this->onBeforeRestCall( $_event );

		if ( $this->_debugLevel > 3 )
			CXLHelperBase

		//	Ok, we've build our request, now let's get the results...
		$_response = YiiXL::makeHttpRequest( $_url, $_queryString, $requestMethod, $this->_userAgent );
		...
		$this->onAfterRestCall( $_event );

		//	If user doesn't want JSON output, then reformat
		switch ( $this->_returnFormat )
		{
			case YiiXL::OF_XML:
				$_response = CXLHelperBase
				break;

			case YiiXL::OF_ASSOC_ARRAY:
	...
	/**
	 * Raises the onBeforeRestCall event
	 *
	 * @param CXLHelperBase
	 */
	public function onBeforeRestCall( CXLHelperBase
	{
		$this->raiseEvent( 'onBeforeRestCall', $event );
	}
	 ...
	 * @return boolean
	 */
	public function beforeRestCall( CXLHelperBase
	{
		return true;
	}

	/**
	 * Raises the onAfterRestCall event. $event contains "raw" return data
	 *
	 * @param CXLHelperBase
	 */
	public function onAfterRestCall( CXLHelperBase
	{
		$this->raiseEvent( 'onAfterRestCall', $event );
	}
	 ...
	 * @return boolean
	 */
	public function afterRestCall( CXLHelperBase
	{
		return true;
	}
	 ...
	 * @param CXLHelperBase
	 */
	public function onRequestComplete( CXLHelperBase
	{
	 ...
	 * @return boolean
	 */
	public function requestComplete( CXLHelperBase
	{
		return true;
	}
