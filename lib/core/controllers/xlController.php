 ...
 */
abstract class xlController extends CController implements xlIController, xlIAccessControl
{
	//********************************************************************************
	//* Constants
	//********************************************************************************
	 ...
	 * @constant string
	 */
	const CLASS_LOG_TAG = 'yiixl.core.components.xlController';
...
SCRIPT;
		...
		if ( XL::UI_JQUERY == ( $_uiStyle = XL::o( $optionList, 'uiStyle', XL::UI_JQUERY ) ) ) xljqUIWrapper::loadScripts();
...
JS;
	 ...
	 */
	/**
	 * 	@defgroup propaccs_cxlcontroller xlController Properties
	 * 	@brief These are the properties accessors for the xlController
	 * 	@{
	 */
	 ...
	 * Maps to {@link xlController::commandMap} and calls the appropriate method.
	 *
	 * @return mixed
	 */
									  ...
									  $bSafeOnly = false )
	{
		...
		if ( isset( $arData, $arData[$_sModelName] ) )
		{
			...
			if ( $model->save() )
			{
				if ( !$bNoCommit && $model instanceof xlModel && $model->hasTransaction() ) $model->commitTransaction();
	...
	protected function _loadPaged( $bSort = false, $oCriteria = null )
	{
		...
		if ( $bSort )
		{
			$_oSort = new xlSort( $this->_modelClass );
